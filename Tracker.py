...
with mp_pose.Pose(
    ...
    while vid.isOpened():
        ...
        try:
            ...
            # neck to mid point
            cv2.line(image, (int(neck_point_x), int(neck_point_y)), (int(based_mid_x), int(based_mid_y)), (line_color),
                     3, cv2.LINE_4)

            # mid to hips
            cv2.line(image, (int(based_mid_x), int(based_mid_y)),
                     (int(left_hip[0] * image_width), (int(left_hip[1] * image_height))), (line_color), 3, cv2.LINE_8)

            cv2.line(image, (int(based_mid_x), int(based_mid_y)),
                     (int(right_hip[0] * image_width), (int(right_hip[1] * image_height))), (line_color), 3, cv2.LINE_8)
            ...
            cv2.putText(image, 'Left Elbow Angle: ' + str(left_arm_angle), (image_width - 290, 70),
                        cv2.FONT_HERSHEY_COMPLEX, 0.7, (0, 255, 255), 2, cv2.LINE_AA)
            cv2.putText(image, 'Right Elbow Angle: ' + str(right_arm_angle), (image_width - 290, 110),
                        cv2.FONT_HERSHEY_COMPLEX, 0.7, (0, 255, 255), 2, cv2.LINE_AA)
            cv2.putText(image, 'Left Knee Angle: ' + str(left_leg_angle), (image_width - 290, 150),
                        cv2.FONT_HERSHEY_COMPLEX, 0.7, (0, 255, 255), 2, cv2.LINE_AA)
            cv2.putText(image, 'Right Knee Angle: ' + str(right_leg_angle), (image_width - 290, 190),
                        cv2.FONT_HERSHEY_COMPLEX, 0.7, (0, 255, 255), 2, cv2.LINE_AA)
